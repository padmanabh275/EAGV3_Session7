...
class OpenAIEmbedder(EmbeddingProvider):
    ...
    async def embed(self, text: str, task_type: TaskType) -> dict:
        ...
        if r.status_code != 200:
            raise EmbedderError(f"openai HTTP {r.status_code}: {r.text[:200]}", status=r.status_code)
        d = r.json()
        rows = d.get("data") or []
        if not rows:
        ...
        if not vec:
            raise EmbedderError(f"openai returned empty embedding: {str(d)[:200]}")
        return {"embedding": vec, "model": self.model, "dim": len(vec)}
...
def build_embedders() -> tuple[list[EmbeddingProvider], list[str]]:
    ...
    fallback_provider = os.getenv("EMBED_FALLBACK_PROVIDER", "openai").lower()
    fallback_model = os.getenv(
        "EMBED_FALLBACK_MODEL",
        "text-embedding-3-small" if fallback_provider == "openai" else "gemini-embedding-001",
    )
    ...
    if fallback_provider == "openai":
        ...
        if key:
            registry["openai"] = OpenAIEmbedder(key, fallback_model)
