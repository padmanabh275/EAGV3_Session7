...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    # crawl4ai/Rich log to stdout/stderr with Unicode (arrows, etc.). On Windows
    # cp1252 that raises UnicodeEncodeError and breaks the tool. Silence both
    # fds during the crawl so nothing corrupts the MCP stdio JSON-RPC stream.
...
def get_time(timezone: str = "UTC") -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            "Asia/Tokyo or Asia/Kolkata. On Windows, install tzdata "
            "(uv add tzdata) if this persists."
        ) from e
    now = datetime.now(tz)
